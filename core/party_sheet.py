...
def _reachable_spells(member: PartyMember, spells: Iterable[Spell]) -> list[Spell]:
    """
    Заклинания, которыми персонаж располагает.

    Если список введён — берём его и только его: до этого роли считались по
    всему списку класса, и для барда с чародеем это было враньё. Они знают
    несколько заклинаний из сотни доступных, а «контроль закрыт» означало лишь,
    что контроль в принципе существует в их классе.

    Введённый список не проверяется на доступный круг: свитки, предметы и
    хоумбрю мастера в него не укладываются, а спорить с игроком о том, что у
    него есть, программе незачем.
    """
    if member.spell_keys:
        return [spell for spell in spells if spell.key in member.spell_keys]
